    ...
    for ($i = 0; $i < $no_files; $i++) {
        ...
        if ($_FILES["files"]["error"][$i] > 0) {
            ...
            if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif" ) {
                ...
                $img = imagejpeg($image, '../upload/resizedAdditionalImages/'.$_FILES["files"]["name"][$i], 75);
                $str .= "<div><img src='".site_name."upload/resizedAdditionalImages/".$_FILES["files"]["name"][$i]."' data-resize='".$_FILES["files"]["name"][$i]."' width='50' height='50' class='resizedImgView'/><a href='#' class='removeImg' data-imgname='<?php echo $i; ?>'><span class='icon-trash-empty'></span> </a></div>";
            }
        }
    }
    echo $str;
